-- Phase 4 Final Patch: HR reporting follows Data Scope independently of mutation rights.

create or replace function public.can_report_hr_case(_user_id uuid,_case_id uuid)
returns boolean language sql stable security definer set search_path=public as $$
  select public.is_hr_user(_user_id) and public.case_access(_user_id,_case_id)<>'none'
$$;
revoke all on function public.can_report_hr_case(uuid,uuid) from public,anon;
grant execute on function public.can_report_hr_case(uuid,uuid) to authenticated;

-- Keep mutation narrower than reporting: HR execution requires Case management
-- for HR Tasks; functional teams execute only their own Task team.
...
returns boolean language sql stable security definer set search_path=public as $$
  select exists(
    select 1 from public.tasks t where t.id=_task_id and (
      t.owner_id=_user_id
      or (t.owner_team='HR' and public.can_manage_case(_user_id,t.case_id))
      or (t.owner_team<>'HR' and public.is_operational_team_member(_user_id,t.owner_team))
    )
  )
$$;
revoke all on function public.can_update_task(uuid,uuid) from public,anon;
...
left join lateral (
  select c.id,c.last_working_day from public.cases c where c.employment_id=e.id and c.case_type='Offboarding'
    ...
    and public.can_report_hr_case(auth.uid(),c.id) order by c.created_at desc limit 1
) oc on true
where e.effective_status in ('active','ending') and p.archived_at is null
  and e.source_onboarding_case_id is not null and public.can_report_hr_case(auth.uid(),e.source_onboarding_case_id);
...
reportable_hr_cases as (
  ...
  where c.archived_at is null and p.archived_at is null
    ...
    and public.can_report_hr_case(auth.uid(),c.id)
),
...
authorized_tasks as (
  select t.id,t.title,t.case_id,c.case_type,c.person_name,c.team_name person_team,
    c.start_date,c.contract_end_date,c.last_working_day,t.due_date,t.priority,t.status,
    t.owner_team,t.owner_id,assignee.name owner_name,t.mandatory,t.completed_at,
    completer.name completed_by_name,c.employment_type,c.status case_status,c.reporting_date
  from public.tasks t join filtered_hr_cases c on c.id=t.case_id
  ...
  where (select hr_reporting from params)
  union all
  select t.id,t.title,t.case_id,t.case_type,t.person_name,t.person_team,
    t.start_date,t.contract_end_date,t.last_working_day,t.due_date,t.priority,t.status,
    t.owner_team,t.owner_id,t.owner_name,t.mandatory,t.completed_at,t.completed_by_name,
    c.employment_type,c.status case_status,
  ...
  where not (select hr_reporting from params)
),
task_rows as (
